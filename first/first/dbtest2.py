...
def search_advance(words_and_f, words_or_f, words_not_f):
    ...
    with sqlite3.connect("db.db") as conn:
        for word in words_and_f:
            words_and_id.append(get_word_id(word, conn))
        for word in words_or_f:
            words_or_id.append(get_word_id(word, conn))
        for word in words_not_f:
            words_not_id.append(get_word_id(word, conn))
        ...
        for main_cnt in range(-1, -main_score_len - 1, -1):
            with sqlite3.connect('db.db') as conn:
                cursor = conn.execute('''SELECT url, title, content FROM pages WHERE id = :page_id ''',
        ...
        return last_result, time.time() - a
...
def search_basic(words_f):
    ...
    with sqlite3.connect('db.db') as conn:
        for word in words:
            words_id.append(get_word_id(word, conn))
